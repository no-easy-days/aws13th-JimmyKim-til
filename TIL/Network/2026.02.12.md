...
1.  Source 정의(Access Control List ~ ACL)
   - access-list <임의의 리스트ID 값> permit <PNAT 대상 내부의 Network Address> <와일드카드마스크>
...
- ***"외부"→"내부"*** !! 목적지를 찍고 들어간다.
  > 사실, Static NAT 기법을 활용한 것이다. 용어를 DNAT라 표현할 뿐!
...
※ 계산 방법
1. 서브넷을 나열하고, ***고정되는 최대 Bit***를 구한다. → 서브네팅과 마찬가지로 네트워크 규모 & Prefix를 구한다.
