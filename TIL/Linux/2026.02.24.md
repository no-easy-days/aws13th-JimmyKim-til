...
1. Kick Start Config 구조 확인
   - cat ~/anaconda.cfg : 이게 결국 GUI에서 클릭클릭하면서 설정했던 행위의 집합체!!! 보면 느낌 온다.
2. Kick Start 파일 생성
   - vi /var/ftp/kick.cfg
3. 생성한 파일 전송을 위해 구성
   - vi /var/lib/tftpboot/pxelinux.cfg/default
     ```
      ...
      LABEL centos7_pxe
      kernel vmlinuz
      APPEND initrd=initrd.img repo=ftp://<FTP서버주소>/pub ks=ftp//<FTP서버주소>/kick.cfg
     ``` 
   > FTP 서버로 보내줘도 되고, 방법은 다양하다. 이걸 ***pxelinux.cfg 에 매핑*** 해주는 것이 포인트!
